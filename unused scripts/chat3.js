...
function typeMessage(element, message, index, interval, callback) {
  ...
  async function handleSubmit(greeting = false) {
    ...
    // Log the final prompt to the console
...
// Lightbox open
$(document).on('click touchstart', '.generated-image, .uploaded-image', function(event) {
...
jQuery(document).ready(function($) {
    // Image upload button touch event
    $('#image-upload-button').on('touchstart click', function() {
        $('#image-upload').click();
    });
...
success: function(response) {
    ...
    // Append the uploaded image to the chat
        var $imageMessage = $('<div class="message user-message"><img src="' + imageUrl + '" alt="Uploaded Image" class="uploaded-image"></div>');
        $(".response-column").append($imageMessage);

    // Append the OpenAI response if available
    if (response.data.openai_response) {
        var $openaiResponseMessage = $('<div class="message ai-message">' + response.data.openai_response + '</div>');
        $(".response-column").append($openaiResponseMessage);
    }

        $(".response-column").animate({ scrollTop: $(".response-column")[0].scrollHeight }, 50);
    } else {
