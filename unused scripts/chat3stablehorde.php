...
function interrogate_image($image_base64) {
    ...
    $headers = array(
        ...
        "Authorization" => "Bearer vLS0oDTp-z7u_K_gr_MZRA"
    );
    ...
    if (is_wp_error($response)) {
        error_log("Error when calling StableHorde API for image interrogation: " . $response->get_error_message());
        return false;
    } else {
        ...
        $request_id = isset($response_body['id']) ? $response_body['id'] : null;

        if ($request_id === null) {
            // If the request ID is not available, log an error and return false
            error_log("Error: Request ID not found in StableHorde API response. Full response: " . print_r($response_body, true));
            return false;
        }
        ...
        while (true) {
            ...
            if ($status === "completed") {
                // If the "message" key exists in the response body
                if (array_key_exists("message", $response_body)) {
                    // Get the message from the response
                    return $response_body['message'];
                } else {
                    // If the "message" key doesn't exist, log an error and return false
                    error_log("Error: 'message' key not found in StableHorde API response for image interrogation. Full response: " . print_r($response_body, true));
                    return false;
                }
            } elseif ($status === "failed") {
                // If the request failed, log an error and return false
                error_log("Error: StableHorde API request failed. Request ID: " . $request_id);
                return false;
            }
...
function get_request_status($request_id) {
    ...
    if (is_wp_error($response)) {
        error_log("Error when calling StableHorde API for request status: " . $response->get_error_message());
        return false;
    } else {
        $response_body = json_decode(wp_remote_retrieve_body($response), true);

        // If the "status" key exists in the response body
        if (array_key_exists("status", $response_body)) {
            // Get the status from the response
            return $response_body['status'];
        } else {
            // If the "status" key doesn't exist, log an error and return false
            error_log("Error: 'status' key not found in StableHorde API response for request status. Full response: " . print_r($response_body, true));
            return false;
        }
    }
}
