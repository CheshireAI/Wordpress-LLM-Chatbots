...
let messageLimit = 0; // Global variable to store the message limit
let isTyping = false;
...
jQuery(document).ready(async ($) => {
  ...
  const responseQueue = [];
  const name = myUserMeta.name || "<?php echo $name; ?>";
  const photoUrl = myUserMeta.photo || "<?php echo $photo; ?>";
  const colors = [
    "#8B008B",
    "#9370DB",
    "#8B008B",
  ...
  const updatePhotoName = (photoUrl, name) => {
    $(".chat-header h3").text(name || "");
    if (photoUrl) {
      $(".chat-header .header-photo, .photo")
        .attr("src", photoUrl)
        .on("load", function () {
          $(".chat-header h3, .chat-header .header-photo, .photo").show();
        });
    } else {
      $(".chat-header .header-photo, .photo").hide();
    }
  };

  updatePhotoName(photoUrl, name);
 ...
 */
  ...
  async function handleSubmit(greeting = false) {
    ...
    if (prompt !== "" || greeting) {
      ...
      $.ajax({
        url: myChatDemoAjax.ajaxurl,
        type: "POST",
        data: {
          action: "generate_chatdemo",
          prompt: prompt,
          greeting: greeting,
        ...
        success: async function (response) {
          ...
          if (userRole !== "administrator") {
            await decreaseMessageLimit();
            console.log("Remaining Message Limit:", messageLimit);
          } else {
            // Bypass message limit checks for administrators
          ...
          // Check the message limit and display alert if necessary
          if (messageLimit <= 0 && userRole !== "administrator") {
            const alertMessage =
              "Sorry babe, you have reached the free user message limit. For 24/7 access to me and other customizable models, join our subscriber's waitlist here <link>.";
            appendAIMessage(alertMessage);
          } else if (messageLimit > 0 && userRole !== "administrator") {
            ai_responses.forEach((ai_response) => {
              //              $responseColumn.find(".placeholder-message").remove(); // Remove the placeholder chat bubble
              responseQueue.push({ response: ai_response.trim() });
            });
  ...
  // Function to decrease the message limit
  async function decreaseMessageLimit() {
    if (userRole === "administrator") {
      return; // Skip message limit decrease for admin users
    }
    ...
    return new Promise((resolve, reject) => {
      $.ajax({
        ...
        type: "POST",
        data: {
          action: "decrease_message_limit",
        },
        success: async function (response) {
          if (response.success) {
            console.log(response.data.message);

            // Fetch the updated message limit
            await getMessageLimit();

            resolve(); // Resolve the promise after successfully decreasing the message limit
          } else {
            console.log(
              "Failed to decrease message limit:",
              response.data.message
            );
            reject(); // Reject the promise if failed to decrease the message limit
          }
        },
        error: function () {
          console.log("Failed to decrease message limit.");
          reject(); // Reject the promise if the AJAX request fails
        },
      });
    });
  }

  // Function to get the message limit from the server
  async function getMessageLimit() {
    return new Promise((resolve, reject) => {
      $.ajax({
        url: myChatDemoAjax.ajaxurl,
        type: "POST",
        data: {
          action: "get_message_limit",
        },
        success: function (response) {
          if (response.success) {
            messageLimit = response.data.messageLimit;
            console.log("Message Limit:", messageLimit);
            resolve(); // Resolve the promise after fetching the updated message limit
          } else {
            console.log("Failed to retrieve message limit.");
            reject(); // Reject the promise if the message limit retrieval fails
          }
        },
        error: function () {
          console.log("Failed to retrieve message limit.");
  ...
  // Retrieve the message limit on page load
